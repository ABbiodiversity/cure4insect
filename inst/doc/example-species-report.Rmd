...
This example species report has been produced using the
...
info[["SpeciesID"]] <- paste0("`\"", info[["ScientificName"]], "\"`")
...
```{r hab-veg,echo=FALSE,fig.height=6,fig.width=12}
...
once for sites with trees present, and onec for sites without trees present. 
Predicted species abundance in each soil/human footprint type is shown with bars. 
Vertical lines indicate 90% confidence intervals.

```{r hab-soil,echo=FALSE,fig.height=6,fig.width=12}
op <- par(mfrow=c(1,2))
...
```{r hab-lin,echo=FALSE,fig.height=6,fig.width=12}
op <- par(mfrow=c(1,2))
...
10 km x 10 km units depicted in red have the higheststandard error, 
while 10 km x 10 km units in dark green have the lowest.

```{r raster-plots,echo=FALSE,fig.height=12,fig.width=8}
...
```{r raster-si2,echo=FALSE,fig.height=6,fig.width=4}
...
```{r secor-plot,echo=FALSE,fig.height=12,fig.width=12}
op <- par(mfrow=c(2,2))
...
```{r secor-multi,echo=FALSE,fig.height=12,fig.width=6}
...
tmp <- plot_sector(z, type="underhf", "Under HF")
points(1:5, h(tmp), pch=4, col=1, cex=3)

tmp <- plot_sector(z, type="unit", "Unit")
points(1:5, h(tmp), pch=4, col=1, cex=3)
par(op)
```
...
Based on thespatial subset, we summarize abundance, intactness, and sector effects.
...
```{r plot-region,echo=FALSE,fig.height=6,fig.width=4}
par(mar=c(1,1,1,1))
...
```{r raster-plots-reg,echo=FALSE,fig.height=6,fig.width=8}
...
```{r secor-plot-reg,echo=FALSE,fig.height=12,fig.width=12}
op <- par(mfrow=c(2,2))
